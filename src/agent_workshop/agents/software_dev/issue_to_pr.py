...
"""
    ...
    async def generate_code(self, state: IssueToPRState) -> IssueToPRState:
        ...
        # Build dynamic style requirements from config
        style_requirements = f"""
## Code Style Requirements (MUST FOLLOW)
1. Formatter: {triangle_config.style.formatter} ({triangle_config.style.line_length} char line length)
2. Linter: {triangle_config.style.linter}
3. Type checker: {triangle_config.style.type_checker}
4. Type hints: All function parameters and return types
5. Docstrings: All public functions/classes with Args/Returns sections
6. Trailing newline: All files must end with a single newline
"""

        # Optionally inject guidelines file content
        if triangle_config.style.guidelines_file:
            guidelines_path = Path(working_dir) / triangle_config.style.guidelines_file
            if guidelines_path.exists():
                try:
                    guidelines_content = guidelines_path.read_text()
                    # Truncate to avoid huge prompts
                    if len(guidelines_content) > 3000:
                        guidelines_content = guidelines_content[:3000] + "\n...(truncated)"
                    style_requirements += f"\n\n## Project Guidelines\n{guidelines_content}"
                except OSError:
        ...
        # Inject style requirements before the Instructions section
    ...
    async def create_pr(self, state: IssueToPRState) -> IssueToPRState:
        ...
        # Load project config for link pattern
        triangle_config = load_triangle_config(working_dir)
        ...
        # Format issue link using config pattern (e.g., "Closes #{issue}")
        issue_link = triangle_config.commits.link_pattern.format(issue=issue_number)
...
## Checklist
- [ ] Code follows project style ({triangle_config.style.formatter}, {triangle_config.style.linter}, type hints)
